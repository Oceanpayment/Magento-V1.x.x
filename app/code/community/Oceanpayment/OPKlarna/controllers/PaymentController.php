 ...
 */
class Oceanpayment_OPKlarna_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses Klarna on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setKlarnaPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opklarna')->__('Customer was redirected to klarna')
		);
		$order->save();
		...
		//获取配置的pay mode
		$standard = Mage::getModel('opklarna/payment');
		...
		if($pay_mode == 1){
			...
			$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opklarna/redirect'));
			$this->renderLayout();	
		}else{
			$this->getResponse()
					->setBody($this->getLayout()
					->createBlock('opklarna/redirect')
					->setOrder($order)
					->toHtml());
		}
	...
	public function noticeAction()
	{
		...
		$model = Mage::getModel('opklarna/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opklarna')->__(self::PUSH.$authType.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opklarna')->__(self::PUSH.$authType.'Payment Failed!'.$history));	
				...
				case -1:
					...
					Mage::helper('opklarna')->__(self::PUSH.'(Pre-auth)Payment Pending!'.$history));
				...
				case '10000':
					...
					Mage::helper('opklarna')->__(self::PUSH.'(High Risk)Payment Failed!'.$history));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opklarna')->__(self::BrowserReturn.'Payment Success!'.$history), true);	
				...
				$order->save();
				$url = 'opklarna/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opklarna')->__(self::BrowserReturn.'Payment Failed!'.$history));					
				
				$order->save();
				$url = 'opklarna/payment/failure';
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opklarna')->__(self::BrowserReturn.'(Pre-auth)Payment Pending!'.$history));
			...
			case '10000':
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_risk'),
				Mage::helper('opklarna')->__(self::BrowserReturn.'(High Risk)Payment Failed!'.$history));
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opklarna/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opklarna/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opklarna/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opklarna')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opklarna')->__('Customer returned from Klarna.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_klarna/loading.gif').'" />';
