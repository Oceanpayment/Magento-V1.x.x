 ...
 * @package 	Oceanpayment_Sofortbanking
 */
class Oceanpayment_OPSofortbanking_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses Sofortbanking on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setSofortbankingPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opsofortbanking')->__('Customer was redirected to sofortbanking')
		);
		$order->save();
		
		$this->getResponse()
		->setBody($this->getLayout()
				->createBlock('opsofortbanking/redirect')
				->setOrder($order)
				->toHtml());
	...
	public function noticeAction()
	{
		...
		$model = Mage::getModel('opsofortbanking/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opsofortbanking')->__(self::PUSH.'Payment Success!
				...
				case 0:
					...
					Mage::helper('opsofortbanking')->__(self::PUSH.'Payment Failed!
    				         (payment_id:'.$_REQUEST['payment_id'].
			    				         ' | order_number:'.$_REQUEST['order_number'].
			    				         ' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
				...
				case -1:
					...
					Mage::helper('opsofortbanking')->__(self::PUSH.'Payment Pending!
    					     (payment_id:'.$_REQUEST['payment_id'].
			    					     ' | order_number:'.$_REQUEST['order_number'].
			    					     ' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
			    					     ' | payment_details:'.$_REQUEST['payment_details'].')')
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opsofortbanking')->__(self::BrowserReturn.'Payment succeed!
						(payment_id:'.$_REQUEST['payment_id'].
						' | order_number:'.$_REQUEST['order_number'].
						' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
						' | payment_details:'.$_REQUEST['payment_details'].')'),true
				...
				$order->save();
				$this->getJslocationreplace(Mage::getUrl('opsofortbanking/payment/success'));
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opsofortbanking')->__(self::BrowserReturn.'Payment failed!
						(payment_id:'.$_REQUEST['payment_id'].
						' | order_number:'.$_REQUEST['order_number'].
						' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
						' | payment_details:'.$_REQUEST['payment_details'].')')
				...
				$order->save();
				$this->getJslocationreplace(Mage::getUrl('opsofortbanking/payment/failure'));
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opsofortbanking')->__(self::BrowserReturn.'Payment Pending!
						(payment_id:'.$_REQUEST['payment_id'].
						' | order_number:'.$_REQUEST['order_number'].
						' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
						' | payment_details:'.$_REQUEST['payment_details'].')')
				...
				$order->save();
				$this->getJslocationreplace(Mage::getUrl('opsofortbanking/payment/pending'));
				break;
			case 20061:
				//订单号重复
				$this->getJslocationreplace(Mage::getUrl('opsofortbanking/payment/failure'));
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opsofortbanking/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opsofortbanking/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opsofortbanking/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opsofortbanking')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opsofortbanking')->__('Customer returned from Sofortbanking.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_sofortbanking/loading.gif').'"  />';
