 ...
 * @package 	Oceanpayment_BROnlineBank
 */
class Oceanpayment_OPBROnlineBank_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses BROnlineBank on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setBROnlineBankPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opbronlinebank')->__('Customer was redirected to bronlinebank')
		);
		$order->save();
		
		$this->getResponse()
		->setBody($this->getLayout()
				->createBlock('opbronlinebank/redirect')
				->setOrder($order)
				->toHtml());
	...
	public function noticeAction()
	{
		...
		$model = Mage::getModel('opbronlinebank/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opbronlinebank')->__(self::PUSH.'Payment Success!
				...
				case 0:
					...
					Mage::helper('opbronlinebank')->__(self::PUSH.'Payment Failed!
    				         (payment_id:'.$_REQUEST['payment_id'].
			    				         ' | order_number:'.$_REQUEST['order_number'].
			    				         ' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
				...
				case -1:
					...
					Mage::helper('opbronlinebank')->__(self::PUSH.'Payment Pending!
    					     (payment_id:'.$_REQUEST['payment_id'].
			    					     ' | order_number:'.$_REQUEST['order_number'].
			    					     ' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
			    					     ' | payment_details:'.$_REQUEST['payment_details'].')')
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opbronlinebank')->__(self::BrowserReturn.'Payment succeed!
						(payment_id:'.$_REQUEST['payment_id'].
						' | order_number:'.$_REQUEST['order_number'].
						' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
						' | payment_details:'.$_REQUEST['payment_details'].')'),true
				...
				$order->save();
				$this->getJslocationreplace(Mage::getUrl('opbronlinebank/payment/success'));
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opbronlinebank')->__(self::BrowserReturn.'Payment failed!
						(payment_id:'.$_REQUEST['payment_id'].
						' | order_number:'.$_REQUEST['order_number'].
						' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
						' | payment_details:'.$_REQUEST['payment_details'].')')
				...
				$order->save();
				$this->getJslocationreplace(Mage::getUrl('opbronlinebank/payment/failure'));
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opbronlinebank')->__(self::BrowserReturn.'Payment Pending!
						(payment_id:'.$_REQUEST['payment_id'].
						' | order_number:'.$_REQUEST['order_number'].
						' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].
						' | payment_details:'.$_REQUEST['payment_details'].')')
				...
				$order->save();
				$this->getJslocationreplace(Mage::getUrl('opbronlinebank/payment/pending'));
				break;
			case 20061:
				//订单号重复
				$this->getJslocationreplace(Mage::getUrl('opbronlinebank/payment/failure'));
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opbronlinebank/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opbronlinebank/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opbronlinebank/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opbronlinebank')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opbronlinebank')->__('Customer returned from BROnlineBank.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_bronlinebank/loading.gif').'"  />';
