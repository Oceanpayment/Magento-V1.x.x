 ...
 */
class Oceanpayment_OPAlipayHK_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses AlipayHK on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setAlipayHKPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opalipayhk')->__('Customer was redirected to AlipayHK')
		);
		$order->save();
		...
		if($pay_mode == 1){
			...
			$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opalipayhk/redirect'));
			$this->renderLayout();	
		}else{
			$this->getResponse()
					->setBody($this->getLayout()
					->createBlock('opalipayhk/redirect')
					->setOrder($order)
					->toHtml());
		}
	...
	public function noticeAction()
	{
		...
		$model = Mage::getModel('opalipayhk/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opalipayhk')->__(self::PUSH.$authType.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opalipayhk')->__(self::PUSH.$authType.'Payment Failed!'.$history));	
				...
				case -1:
					...
					Mage::helper('opalipayhk')->__(self::PUSH.'(Pre-auth)Payment Pending!'.$history));
				...
				case '10000':
					...
					Mage::helper('opalipayhk')->__(self::PUSH.'(High Risk)Payment Failed!'.$history));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opalipayhk')->__(self::BrowserReturn.'Payment Success!'.$history), true);	
				...
				$order->save();
				$url = 'opalipayhk/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opalipayhk')->__(self::BrowserReturn.'Payment Failed!'.$history));					
				
				$order->save();
				$url = 'opalipayhk/payment/failure';
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opalipayhk')->__(self::BrowserReturn.'(Pre-auth)Payment Pending!'.$history));
				...
				if($_REQUEST['payment_authType'] == 1){
					...
					$url = 'opalipayhk/payment/pending';
				}
				break;
			case '10000':
				//10000:Payment is declined 高风险订单
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_risk'),
				Mage::helper('opalipayhk')->__(self::BrowserReturn.'(High Risk)Payment Failed!'.$history));
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opalipayhk/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opalipayhk/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opalipayhk/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opalipayhk')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opalipayhk')->__('Customer returned from AlipayHK.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_alipayhk/loading.gif').'" />';
