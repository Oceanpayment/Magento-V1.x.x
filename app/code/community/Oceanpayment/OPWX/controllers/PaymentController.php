 ...
 */
class Oceanpayment_OPWX_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses WX on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setWXPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opwx')->__('Customer was redirected to wx')
		);
		$order->save();
		
		//获取配置的pay mode
		$standard = Mage::getModel('opwx/payment');
		...
		if($pay_mode == 1){
		    ...
		    $this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opwx/redirect'));
		    $this->renderLayout();
		}else{
		    $this->getResponse()
		    ->setBody($this->getLayout()
		        ->createBlock('opwx/redirect')
		        ->setOrder($order)
		        ->toHtml());
		}
	...
	//获取推送输入流XML
		...
		$model = Mage::getModel('opwx/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opwx')->__(self::PUSH.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opwx')->__(self::PUSH.'Payment Failed!'.$history));
						
					$order->save();
					break;
				case -1:
					//交易待处理
					$url = 'https://print.ebanx.com/print/wx/?hash='.$_REQUEST['pay_barCode'];

					$order->addStatusToHistory(
					$model->getConfigData('order_status_payment_pending'),
					Mage::helper('opwx')->__(self::PUSH.'Payment Pending!'.$history.' | Url:'.$url));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opwx')->__(self::BrowserReturn.'Payment Success!'.$history), true);
				...
				$order->save();
				$url = 'opwx/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opwx')->__(self::BrowserReturn.'Payment Failed!'.$history));
				
				$order->save();
				$url = 'opwx/payment/failure';
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opwx')->__(self::BrowserReturn.'Payment Pending!'.$history));
				
				$order->save();
				$url = 'opwx/payment/pending';
				break;
			case 20061:
				//订单号重复
				$url = 'opwx/payment/failure';
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opwx/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opwx/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opwx/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opwx')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opwx')->__('Customer returned from WX.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_wx/loading.gif').'"  />';
