 ...
 */
class Oceanpayment_OPUnionPay_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	public function redirectAction()
	{
		...
		$order->getStatus(),
		Mage::helper('opunionpay')->__('Customer was redirected to unionpay')
		);
		$order->save();
		...
		//获取配置的pay mode
		$standard = Mage::getModel('opunionpay/payment');
		...
		if($pay_mode == 1){
			...
			$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opunionpay/redirect'));
			$this->renderLayout();	
		}else{
			$this->getResponse()
					->setBody($this->getLayout()
					->createBlock('opunionpay/redirect')
					->setOrder($order)
					->toHtml());
		}
	...
	public function noticeAction()
	{
		...
		$model = Mage::getModel('opunionpay/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opunionpay')->__(self::PUSH.$authType.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opunionpay')->__(self::PUSH.$authType.'Payment Failed!'.$history));	
				...
				case -1:
					...
					Mage::helper('opunionpay')->__(self::PUSH.'(Pre-auth)Payment Pending!'.$history));
				...
				case '10000':
					...
					Mage::helper('opunionpay')->__(self::PUSH.'(High Risk)Payment Failed!'.$history));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opunionpay')->__(self::BrowserReturn.'Payment Success!'.$history), true);	
				...
				$order->save();
				$url = 'opunionpay/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opunionpay')->__(self::BrowserReturn.'Payment Failed!'.$history));					
				
				$order->save();
				$url = 'opunionpay/payment/failure';
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opunionpay')->__(self::BrowserReturn.'(Pre-auth)Payment Pending!'.$history));
				...
				if($_REQUEST['payment_authType'] == 1){
					...
					$url = 'opunionpay/payment/pending';
				}
				break;
			case '10000':
				//10000:Payment is declined 高风险订单
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_risk'),
				Mage::helper('opunionpay')->__(self::BrowserReturn.'(High Risk)Payment Failed!'.$history));
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opunionpay/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opunionpay/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opunionpay/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opunionpay')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opunionpay')->__('Customer returned from UnionPay.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_unionpay/loading.gif').'" />';
