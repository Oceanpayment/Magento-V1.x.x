 ...
 */
class Oceanpayment_OPOXXO_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses OXXO on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setOXXOPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opoxxo')->__('Customer was redirected to oxxo')
		);
		$order->save();
		
		$this->getResponse()
		->setBody($this->getLayout()
				->createBlock('opoxxo/redirect')
				->setOrder($order)
				->toHtml());
	...
	//获取推送输入流XML
		...
		$model = Mage::getModel('opoxxo/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opoxxo')->__(self::PUSH.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opoxxo')->__(self::PUSH.'Payment Failed!'.$history));
						
					$order->save();
					break;
				case -1:
					//交易待处理
					$url = 'https://print.ebanx.com/print/oxxo/?hash='.$_REQUEST['pay_barCode'];

					$order->addStatusToHistory(
					$model->getConfigData('order_status_payment_pending'),
					Mage::helper('opoxxo')->__(self::PUSH.'Payment Pending!'.$history.' | Url:'.$url));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opoxxo')->__(self::BrowserReturn.'Payment Success!'.$history), true);
				...
				$order->save();
				$url = 'opoxxo/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opoxxo')->__(self::BrowserReturn.'Payment Failed!'.$history));
				
				$order->save();
				$url = 'opoxxo/payment/failure';
				break;
			case -1:
				//交易待处理
				$url = 'https://print.ebanx.com/print/oxxo/?hash='.$_REQUEST['pay_barCode'];
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opoxxo')->__(self::BrowserReturn.'Payment Pending!'.$history.' | Url:'.$url));
				
				$order->save();
				
				$this->getJsLocationReplace($url);
				exit;
				//$url = 'opoxxo/payment/pending';
				break;
			case 20061:
				//订单号重复
				$url = 'opoxxo/payment/failure';
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opoxxo/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opoxxo/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opoxxo/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opoxxo')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opoxxo')->__('Customer returned from OXXO.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_oxxo/loading.gif').'"  />';
