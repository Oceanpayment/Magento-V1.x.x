 ...
 */
class Oceanpayment_OPAlipay_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	public function redirectAction()
	{
		...
		$order->getStatus(),
		Mage::helper('opalipay')->__('Customer was redirected to alipay')
		);
		$order->save();
	...
	//获取推送输入流XML
		...
		$model = Mage::getModel('opalipay/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opalipay')->__(self::PUSH.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opalipay')->__(self::PUSH.'Payment Failed!'.$history));
						
					$order->save();
					break;
				case -1:
					//交易待处理
					$url = 'https://print.ebanx.com/print/alipay/?hash='.$_REQUEST['pay_barCode'];

					$order->addStatusToHistory(
					$model->getConfigData('order_status_payment_pending'),
					Mage::helper('opalipay')->__(self::PUSH.'Payment Pending!'.$history.' | Url:'.$url));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opalipay')->__(self::BrowserReturn.'Payment Success!'.$history), true);
				...
				$order->save();
				$url = 'opalipay/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opalipay')->__(self::BrowserReturn.'Payment Failed!'.$history));
				
				$order->save();
				$url = 'opalipay/payment/failure';
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opalipay')->__(self::BrowserReturn.'Payment Pending!'.$history));
				
				$order->save();
				$url = 'opalipay/payment/pending';
				break;
			case 20061:
				//订单号重复
				$url = 'opalipay/payment/failure';
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opalipay/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opalipay/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opalipay/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opalipay')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opalipay')->__('Customer returned from Alipay.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_alipay/loading.gif').'"  />';
