 ...
 */
class Oceanpayment_OPBoleto_Model_Payment extends Mage_Payment_Model_Method_Abstract
{
    protected $_code  = 'oceanpayment_boleto';
    protected $_formBlockType = 'opboleto/form';

    // Boleto return codes of payment
	...
	{
		return Mage::getUrl('opboleto/payment/return', array('_secure' => true,'_nosid' => true));
	}

	/**
	 *  Return URL for Boleto success response
	 *
	 *  @return	  string URL
	 */
    ...
    /**
     *  Return URL for Boleto failure response
     *
     *  @return	  string URL
     */
    protected function getErrorURL()
    {
        return Mage::getUrl('opboleto/payment/error', array('_secure' => true));
    }

	/**
	 *  Return URL for Boleto notify response
	 *
	 *  @return	  string URL
	 */
	protected function getNoticeURL()
	{
		return Mage::getUrl('opboleto/payment/notice', array('_secure' => true,'_nosid' => true));
	}
    ...
    {
        $block = $this->getLayout()->createBlock('opboleto/form_payment', $name);
    ...
    {
        return Mage::getUrl('opboleto/payment/redirect', array('_secure' => true));
    }
    ...
    public function assignData($data)
    {
    	...
    	$pay_type = $data->getData('pay_type');
    	$pay_userName = $data->getData('pay_userName');
    	$pay_email = $data->getData('pay_email');
    	$pay_cnpj = $data->getData('pay_cnpj');
    	$pay_cpf = $data->getData('pay_cpf');
    	$pay_cnpj_pass = $data->getData('pay_cnpj_pass');
    	$pay_cpf_pass = $data->getData('pay_cpf_pass');
    
    	Mage::getSingleton('checkout/session')->setData('pay_type', $pay_type);
    	Mage::getSingleton('checkout/session')->setData('pay_userName', $pay_userName);
    	Mage::getSingleton('checkout/session')->setData('pay_email', $pay_email);
    	Mage::getSingleton('checkout/session')->setData('pay_cnpj', $pay_cnpj);
    	Mage::getSingleton('checkout/session')->setData('pay_cpf', $pay_cpf);
    	Mage::getSingleton('checkout/session')->setData('pay_cnpj_pass', $pay_cnpj_pass);
    	Mage::getSingleton('checkout/session')->setData('pay_cpf_pass', $pay_cpf_pass);
    ...
    public function validate()
    {
    	...
    	*/
    	...
    	//判断CNPJ是否正确
    	$pay_cnpj_pass = $session->getData('pay_cnpj_pass') != 0 ? $session->getData('pay_cnpj_pass') : 0;
    	if(!$pay_cnpj_pass){
    		$errorMsg = Mage::helper('payment')->__('The CNPJ is incorrect.');
    		Mage::throwException($errorMsg);
    	}
    	
    	//判断CPF是否正确
    	$pay_cpf_pass = $session->getData('pay_cpf_pass') != 0 ? $session->getData('pay_cpf_pass') : 0;	
    	if(!$pay_cpf_pass){
    		$errorMsg = Mage::helper('payment')->__('The CPF is incorrect.');
    		Mage::throwException($errorMsg);
    	}
    ...
    /**
     *  Return Standard Checkout Form Fields for request to Boleto
     *
    ...
    public function getStandardCheckoutFormFields()
    {
		...
		//付款人名称
		$pay_userName  	   = $session->getData('pay_userName') != '' ? $session->getData('pay_userName') : '';
		//付款人邮箱
		$pay_email         = $session->getData('pay_email') != '' ? $session->getData('pay_email') : '';
		//付款人CPF
		$pay_cpf           = $session->getData('pay_cpf') != '' ? $session->getData('pay_cpf') : '';	
		//公司CNPJ
		if($session->getData('pay_type') == 'CNPJ'){
			$pay_cnpj	   = $session->getData('pay_cnpj') != '' ? $session->getData('pay_cnpj') : '';
		}else{
			$pay_cnpj	   = '';
		}
		//Boleto类型
	...
	/**
	 * Return authorized languages by Boleto
	 *
	 * @param	none
	 * @return	array
	 */
	protected function _getAuthorizedLanguages()
	{
        ...
        foreach (Mage::getConfig()->getNode('global/payment/boleto_payment/languages')->asArray() as $data) 
		{
	...
	/**
	 * Return language code to send to Boleto
	 *
	 * @param	none
	 * @return	String
	 */
    ...
    /**
     *  Return response for Boleto success payment
     *
     *  @param    none
    ...
    /**
     *  Return response for Boleto failure payment
     *
     *  @param    none
