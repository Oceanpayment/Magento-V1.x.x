 ...
 */
class Oceanpayment_OPBoleto_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses Boleto on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setBoletoPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opboleto')->__('Customer was redirected to boleto')
		);
		$order->save();
		
		$this->getResponse()
		->setBody($this->getLayout()
				->createBlock('opboleto/redirect')
				->setOrder($order)
				->toHtml());
	...
	public function noticeAction()
	{
		...
		$model = Mage::getModel('opboleto/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			$history = ' (payment_id:'.$_REQUEST['payment_id'].' | order_number:'.$_REQUEST['order_number'].' | '.$_REQUEST['order_currency'].':'.$_REQUEST['order_amount'].' | payment_details:'.$_REQUEST['payment_details'].' | barCode:'.$_REQUEST['pay_barCode'].')';
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opboleto')->__(self::PUSH.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opboleto')->__(self::PUSH.'Payment Failed!'.$history));
				...
				case -1:
					...
					Mage::helper('opboleto')->__(self::PUSH.'Payment Pending!'.$history));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opboleto')->__(self::BrowserReturn.'Payment Success!'.$history), true);
				...
				$order->save();
				$url = 'opboleto/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opboleto')->__(self::BrowserReturn.'Payment Failed!'.$history));
				
				$order->save();
				$url = 'opboleto/payment/failure';
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opboleto')->__(self::BrowserReturn.'Payment Pending!'.$history));
				
				$order->save();
				$url = 'opboleto/payment/pending';
				break;
			case 2:
				//该订单已经支付成功
				$url = 'opboleto/payment/success';
	...
	private function _validated()
	{
		...
		Mage::getSingleton('checkout/session')->setData('pay_barCode', $pay_barCode);	
		Mage::getSingleton('checkout/session')->setData('errorCode', $errorCode);
		
		$session->unsetData('pay_type');
		$session->unsetData('pay_userName');
		$session->unsetData('pay_email');
		$session->unsetData('pay_cnpj');
		$session->unsetData('pay_cpf');
		$session->unsetData('pay_cnpj_pass');
		$session->unsetData('pay_cpf_pass');
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opboleto/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opboleto/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opboleto/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opboleto')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opboleto')->__('Customer returned from Boleto.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_boleto/loading.gif').'"  />';
